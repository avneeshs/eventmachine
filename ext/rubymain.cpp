...
static VALUE t_set_tls_parms (VALUE self, VALUE signature, VALUE privkeyfile, VALUE certchainfile, VALUE verify_peer, VALUE cafile, VALUE verify_depth)
{
	 ...
	 */
	int depth;
	if (verify_depth == Qnil) {
		depth = -1;
	} else {
		depth = FIX2INT(verify_depth);
	}

	evma_set_tls_parms (NUM2ULONG (signature), StringValuePtr (privkeyfile), StringValuePtr (certchainfile), (verify_peer == Qtrue ? 1 : 0), StringValuePtr (cafile), depth);
	return Qnil;
}
...
/***********************
t_get_peer_verify_error
***********************/
static VALUE t_get_peer_verify_error (VALUE self, VALUE signature)
{
	VALUE ret = Qnil;
	char buf[80];
	size_t len;

	len = snprintf(buf, sizeof(buf), "SSL not enabled");

    #ifdef WITH_SSL
	len = sizeof(buf);
	if (evma_get_peer_verify_result (NUM2ULONG (signature), buf, &len)) {
		return Qnil;
	} 
	#endif
...
extern "C" void Init_rubyeventmachine()
{
	...
	rb_define_module_function (EmModule, "get_peer_verify_error", (VALUE(*)(...))t_get_peer_verify_error, 1);
