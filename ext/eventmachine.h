...
extern "C" {
	...
	void evma_set_tls_parms (const unsigned long binding, const char *privatekey_filename, const char *certchain_filenane, int verify_peer, const char *ca_filename, int verify_depth);
	void evma_start_tls (const unsigned long binding);

	#ifdef WITH_SSL
	X509 *evma_get_peer_cert (const unsigned long binding);
	int evma_get_peer_verify_result (const unsigned long binding, char *err_buf, size_t *err_buf_len);
